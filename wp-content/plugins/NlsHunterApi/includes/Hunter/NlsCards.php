...
class NlsCards extends NlsService
{
    ...
    {
        $this->url = get_option(NlsHunterApi_Admin::CARDS_WSDL_URL);
        parent::init();
    }
    ...
    public function applicantsGetByBasket($basketGuid) 
    {
        ...
        try {
            $params = [
                'languageId' => $this->settings->languageCode,
                'basketId' => $basketGuid,
...
# Supplier
        //$this->jobFilterEntry($filter, get_option(NlsHunterApi_Admin::NSOFT_SUPPLIER_ID), "SupplierId");
# jobCode
